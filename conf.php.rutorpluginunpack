...
$deleteAutoArchives = true;	// Delete archive files after successful auto unpack. Will not remove archives unless AutoMove is enabled and the operation type is not Move.
$unpackToTemp = true;    // During auto upacking, the archive will be unpacked in the $tempDirectory defined in rutorrent/conf/config.php, and then will be moved from there to the final destination.
